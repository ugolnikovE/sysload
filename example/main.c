...
int main() {
    ...
    if (cpu_ret == 0) {
        ...
        printf("Warning: Failed to get CPU usage (return code: %d)\n", cpu_ret);
    }
    ...
    if (storage_ret == 0 && rootfs_info.total > 0) {
        ...
        printf("Warning: Failed to get storage info (return code: %d)\n", storage_ret);
    }
    ...
    if (uptime_ret == 0) {
        ...
        printf("Warning: Failed to get uptime info (return code: %d)\n", uptime_ret);
    }
